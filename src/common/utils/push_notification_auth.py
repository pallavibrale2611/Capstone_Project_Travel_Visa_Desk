"""Push notification authentication utilities."""
...
class PushNotificationAuth:
    ...
    def _calculate_request_body_sha256(self, data: dict[str, Any]):
        """Calculate the SHA256 hash of a request body.
...
class PushNotificationSenderAuth(PushNotificationAuth):
    """Authentication for sending push notifications."""
    ...
    def _generate_jwt(self, data: dict[str, Any]):
        """Generate JWT by signing both the request payload SHA digest and time of token generation.
...
class PushNotificationReceiverAuth(PushNotificationAuth):
    """Authentication for receiving push notifications."""
