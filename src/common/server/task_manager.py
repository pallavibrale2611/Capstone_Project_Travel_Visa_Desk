"""Task manager implementation for handling A2A tasks."""
...
class InMemoryTaskManager(TaskManager):
    """In-memory implementation of task manager."""
    ...
    async def has_push_notification_info(self, task_id: str) -> bool:
        """Check if task has push notification info."""
        async with self.lock:
    ...
    async def upsert_task(self, task_send_params: TaskSendParams) -> Task:
        ...
        async with self.lock:
            ...
            if task is None:
                ...
                print(f"Task : {task}")
                print(f"Task_send_param:{task_send_params}")
    ...
    def append_task_history(self, task: Task, historyLength: int | None):
        """Append task history with optional length limit."""
